...
export const DoctorLayout = () => {
  ...
  const tagLineStyle = { opacity: 0.75 }
  ...
  const menuItems = [
    ...
    { name: "Appointments", path: "/doctor/appointments", icon: CalendarClock },
    { name: "Prescriptions", path: "/doctor/prescriptions", icon: ClipboardList },
    { name: "Patients", path: "/doctor/patients", icon: UsersRound },
    { name: "Availability", path: "/doctor/availability", icon: Stethoscope },
  ]
  ...
  return (
    <div className={darkMode ? "dark" : ""}>
      <div className="flex h-screen bg-background">
        {/* Sidebar */}
        <div
          className={`${sidebarOpen ? "w-72" : "w-24"} relative flex h-full flex-col overflow-hidden border-r border-border bg-card text-foreground transition-[width] duration-300`}
        >
            ...
            >
              ...
              {sidebarOpen && (
                <div className="leading-tight">
                  <p className="text-sm font-semibold tracking-wide">Smart Healthcare</p>
                  <p className="text-xs" style={tagLineStyle}>
                    Management System
                  </p>
                </div>
              )}
          ...
          <nav className="flex flex-1 flex-col gap-1 px-3 py-4 pb-24">
            {menuItems.map((item) => {
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between border-b border-border bg-card px-6 py-4 shadow-sm">
            <div className="flex items-center gap-3">
              <button
                onClick={() => setSidebarOpen(!sidebarOpen)}
                className="rounded-lg border border-border/60 bg-card px-2 py-2 text-muted-foreground transition hover:bg-muted/40 hover:text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 md:hidden"
                aria-label={sidebarOpen ? "Collapse sidebar" : "Expand sidebar"}
              >
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-border bg-card px-3 py-2 shadow-sm">
                <div className="hidden text-right leading-tight sm:flex sm:flex-col sm:items-end">
                  ...
                  <p className="text-xs capitalize text-muted-foreground">{user?.role ?? "doctor"}</p>
                </div>
                <div
